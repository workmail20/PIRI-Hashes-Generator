...
namespace PIRIHashesGenerator.units.pdf_templates
{
    ...
    public class TransactionsDocument : IDocument
    {
        ...
        public TransactionsDocument(ListView _lw_)
        {
        ...
        void ComposeHeader(IContainer container)
        {
            ...
            container.Row(row =>
            {
                ...
                {
                    column.Item().Text("PIRI Transactions").Style(titleStyle);
        ...
        void ComposeTable(IContainer container)
        {
            container.Table(table =>
            {
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(CellStyle).Text("Amount").FontFamily("Shippori Antique").ExtraBlack().FontSize(12);
                    header.Cell().Element(CellStyle).Text("Type").FontFamily("Shippori Antique").ExtraBlack().FontSize(12);
                ...
                foreach (ListViewItem item in _lw.Items)
                {
                    if (item.Checked)
                    {
                        table.Cell().ShowEntire().BorderRight(1).BorderColor(Colors.Grey.Lighten2).Element(CellStyle).Text("Tx hash: " + item.SubItems[2].Text + "\r\nFrom: " + item.SubItems[3].Text + "\r\nTo: " + item.SubItems[4].Text).FontSize(8).FontFamily("Shippori Antique");
                        table.Cell().ShowEntire().BorderRight(1).BorderColor(Colors.Grey.Lighten2).Element(CellStyle).Text(item.SubItems[5].Text).FontSize(8).FontFamily("Shippori Antique");
                        table.Cell().ShowEntire().Element(CellStyle).Text("[" + item.SubItems[1].Text + "] " + item.SubItems[6].Text).FontSize(8).FontFamily("Shippori Antique");
